...
public class DeptServiceImpl extends ServiceImpl<DeptMapper, Dept> implements IDeptService {
    ...
    public InterfaceResult<String> addDeptUser(Dept dept,UserInfo userInfo){
        ...
        String deptCode  = UUID.randomUUID().toString().replaceAll("-","");
        dept.setDeptCode(deptCode);
        dept.setCreateTime(LocalDateTime.now());
        int deptStatus = deptMapper.insert(dept);
        if (deptStatus > 0){
