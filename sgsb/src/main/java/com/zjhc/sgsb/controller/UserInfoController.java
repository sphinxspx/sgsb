...
public class UserInfoController extends BaseController {
    ...
    public InterfaceResult<String> resetPassword(String token,UserInfo userInfo,String newPassword){
        InterfaceResult<UserInfo> authentication = userInfoService.authentication(token);
        if (!authentication.issuccess()){
            return InterfaceResult.getError(authentication.getMsg());
        }
        return userInfoService.resetPassword(userInfo,newPassword);
    }
